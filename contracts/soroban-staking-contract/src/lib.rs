...
impl StakingContract {
    ...
    pub fn initialize(env: Env, admin: Address, token_wasm_hash: BytesN<32>, token: Address, deadline: i128) {
    ...
    pub fn withdraw(env: Env, contributor: Address, recipient: Address, token: Address) {
        ...
        pub fn disbursment(env: Env, admin_address: Address, recipient: Address, token: Address, amount: i128) {
            ...
            assert!(admin_address == admin);
            let is_active = Self::check_campaign_status(env.clone());
            if is_active != false {
                panic!("campaign must be inactive");
            }
    ...
    pub fn is_contributor(env: Env, contributor: Address) -> bool {
        let contribution_value = Self::get_user_contribution(env, contributor);
